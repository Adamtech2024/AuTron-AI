...
log = logging.getLogger("autron-nano")
...
As Nano (Standard), you should:
- Prioritize clarity and efficiency in every response.
- Get straight to the point without unnecessary preamble.
- Keep responses focused and practical -- answer the question, provide key details, move on.
...
AuTron Nano maintains a clear, efficient, and friendly tone. It gets to the point quickly. It avoids unnecessary formatting and keeps responses tight.
...
# ===============================================================================
# CONFIG - PERFORMANCE NANO
# ===============================================================================
MODELS = {
    "fast": "sam860/LFM2:350m",
    "think": ["sam860/LFM2:700m", "sam860/LFM2:350m"],
    "pro": ["sam860/LFM2:700m", "sam860/LFM2:700m"],
    "default": "sam860/LFM2:350m"
}
VERSION = "AuTron 4 Nano (Standard)"
DATA_DIR = Path(__file__).parent / ".autron-storage"
KNOWLEDGE_FILE = DATA_DIR / "knowledge_nano.json.gz"
HISTORY_FILE = DATA_DIR / "history_nano.json"
...
class AuTron:
    ...
    def init(self):
        out.clear()
        out.print(f"\n    ╔═══════════════════════════════════════════════════════════════╗")
        out.print(f"    ║                 ⚙️ {VERSION} ⚙️                  ║")
        out.print(f"    ╚═══════════════════════════════════════════════════════════════╝\n", style="bold white")
        if not OLLAMA_AVAILABLE:
            out.print("❌ Ollama not running")
            return False
        out.print(f"⚙️ Ready | 🌐 {'Online' if is_online() else 'Offline'}")
...
**/pro** ⚙️ <q> - Performance Dual-Brain
**/think** <q> - Efficient Synthesis
**/fast** <q> - Instant Response (350M)

[ INTELLIGENCE ]
**/train** [time] - Efficiency Optimization
...
**/exit** - Shutdown"""
            ...
            if self.last_results: out.print(f"\n💬 Processing Insight:\n{self.last_results[-1]}", style="italic white")
